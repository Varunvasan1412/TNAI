...
const MenuItem = ({ item, depth = 0, openMenus, toggleMenu, currentPath, isCollapsed }) => {
  const Icon = item.icon;
  const hasChildren = item.children && item.children.length > 0;
  const isActive = item.path === currentPath || (item.path && item.path !== '/' && currentPath.startsWith(item.path + '/'));
  const isOpen = openMenus[item.id];

  const isChildActive = useCallback((items) => {
  ...
  const paddingLeft = isCollapsed 
    ? 'px-0 justify-center' 
    : (depth === 0 ? 'pl-4' : depth === 1 ? 'pl-10' : 'pl-14');
        ...
        >
          {Icon && (
            <Icon className={cn(
              ...
              isActive ? 'text-white' : 'text-gray-400 group-hover:text-primary dark:group-hover:text-emerald-400'
            )} />
          )}
  ...
  return (
    <li className="relative">
      <button
        ...
        style={(isOpen || hasActiveChild) && !isCollapsed ? { background: 'linear-gradient(135deg, var(--color-primary) 0%, var(--color-secondary) 100%)' } : {}}
      >
        {Icon && (
          <Icon className={cn(
            'w-[18px] h-[18px] flex-shrink-0 transition-colors',
            (isOpen || hasActiveChild) && !isCollapsed ? 'text-white' : 'text-gray-400 group-hover:text-primary dark:group-hover:text-emerald-400'
          )} />
        )}
        {!isCollapsed && <span className="flex-1 truncate">{item.label}</span>}
        {!isCollapsed && item.badge && <MenuBadge badge={item.badge} />}
      ...
      for (const item of items) {
        if (item.children) {
          const hasMatch = item.children.some(
            (c) => c.path === path || (c.children && c.children.some((gc) => gc.path === path))
          );
          if (hasMatch) {
    ...
    <>
      <aside
        className={cn(
          ...
          isOpen ? 'w-[250px] translate-x-0' : 'w-[250px] lg:w-[70px] -translate-x-full lg:translate-x-0',
        )}
      >
        {/* Logo */}
        <div className="flex items-center gap-3 h-16 px-5 border-b border-gray-100 dark:border-slate-700">
          <Link to="/dashboard" className="flex items-center gap-2.5">
            <img src="/assets/images/favicon.png" alt="Logo" className="h-7 w-7" />
            {isOpen && (
