...
const ACTIVE_FROM = 'var(--color-primary)';
const ACTIVE_TO   = 'var(--color-secondary)';
...
const MenuBadge = ({ badge }) => {
  ...
  return (
    <span className={cn("ml-auto px-1.5 py-0.5 text-[10px] font-semibold rounded", colors[badge.variant] || colors.danger)}>
      {badge.text}
    </span>
  );
};
...
const MenuItem = ({ item, depth = 0, openMenus, toggleMenu, currentPath, isCollapsed }) => {
  ...
  const isChildActive = useCallback(
    (items) => {
      ...
      return items.some((child) => child.path === currentPath || isChildActive(child.children));
    },
    [currentPath],
  );
      ...
      <li className="relative">
        {isActive && (
          <span
            ...
            style={{ background: `linear-gradient(180deg, ${ACTIVE_FROM}, ${ACTIVE_TO})` }}
          />
        )}
        <Link
          to={item.path || "#"}
          ...
          style={isActive ? { background: `linear-gradient(135deg, ${ACTIVE_FROM} 0%, ${ACTIVE_TO} 100%)` } : {}}
        >
          {Icon && (
            <Icon
              className={cn(
        ...
        if (item.children) {
          ...
          if (hasMatch) {
            ...
            item.children.forEach((c) => {
              if (c.children && c.children.some((gc) => gc.path === path)) initial[c.id] = true;
            });
          }
        }
      }
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-center h-20 border-b border-gray-100 dark:border-slate-700">
          <Link to="/products/dashboard" className="flex items-center gap-2.5 overflow-hidden">
