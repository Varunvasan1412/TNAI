...
const Statistics = () => {
  ...
  const { data, fetchAll, loading, deleteItem } = useStatisticStore();
  ...
  return (
    <div className="relative space-y-7 min-h-screen">
      ...
      <div className="relative z-10">
        <PageTitle title="Statistics" breadcrumbs={[{ label: 'CRM', path: '/admin/dashboard' }, { label: 'Statistics', active: true }]} />
      </div>
        ...
        <Card className="overflow-hidden bg-white/75 dark:bg-slate-800/65 backdrop-blur-2xl">
          <CardHeader>
            <div className="flex justify-between items-center">
              ...
              <button onClick={() => navigate('/admin/statistics/add')} className="flex items-center gap-1.5 h-9 px-4 text-white text-[13px] font-bold rounded-xl shadow-lg" style={{ background: 'linear-gradient(135deg, var(--color-primary), var(--color-secondary))' }}>
              ...
              <TBody>
                {loading ? <TR><TD colSpan={5} className="text-center py-10">Loading...</TD></TR> : data.map((item, idx) => (
                  <TR key={item.id}>
                    ...
                    <TD className="font-semibold text-primary">{item.value}</TD>
                    <TD><Badge variant={item.status === 'Active' ? 'soft-success' : 'soft-danger'}>{item.status || 'Active'}</Badge></TD>
                    <TD className="text-right">
                      <div className="flex justify-end gap-1">
                        <button onClick={() => navigate(`/admin/statistics/view/${item.id}`)} className="w-8 h-8 flex justify-center items-center hover:bg-info/10 text-gray-400 hover:text-info rounded-lg"><Feather.Eye className="w-4 h-4" /></button>
                        <button onClick={() => navigate(`/admin/statistics/edit/${item.id}`)} className="w-8 h-8 flex justify-center items-center hover:bg-primary/10 text-gray-400 hover:text-primary rounded-lg"><Feather.Edit2 className="w-4 h-4" /></button>
