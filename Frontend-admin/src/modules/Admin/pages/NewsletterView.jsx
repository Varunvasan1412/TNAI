...
const NewsletterView = () => {
  ...
  const { single, fetchSingle, loading } = useNewsletterStore();
  ...
  return (
    <div className="relative space-y-7 min-h-screen">
      ...
      <div className="relative z-10 flex items-center justify-between">
        <PageTitle title="View Record" breadcrumbs={[{ label: 'CRM', path: '/admin/dashboard' }, { label: 'Newsletters', path: '/admin/newsletter' }, { label: 'Details', active: true }]} />
        <div className="flex gap-2">
          ...
          <Button size="sm" onClick={() => navigate(`/admin/newsletter/edit/${id}`)} style={{ background: 'linear-gradient(135deg, var(--color-primary), var(--color-secondary))', color: 'white', border: 'none' }}>
            ...
            <CardBody className="p-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                ...
                <div className="flex flex-col p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                  <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">Newsletter Title</span>
                  <span className="text-[14px] font-semibold text-slate-800 dark:text-slate-200">{single.title || '-'}</span>
                </div>
                <div className="flex flex-col p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                  <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">Issue/Volume</span>
                  <span className="text-[14px] font-semibold text-slate-800 dark:text-slate-200">{single.issue_volume || '-'}</span>
                </div>
                <div className="flex flex-col p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                  <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">Publication Date</span>
                  <span className="text-[14px] font-semibold text-slate-800 dark:text-slate-200">{single.publication_date || '-'}</span>
                </div>
                <div className="flex flex-col p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                  <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">Cover Image</span>
                  {single.cover_image ? (
                    <img src={single.cover_image} alt={single.title} className="mt-1 h-16 w-auto rounded border" />
                  ) : (
                ...
                <div className="flex flex-col p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                  <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">Newsletter PDF</span>
                  {single.newsletter_pdf ? (
                    <a href={single.newsletter_pdf} target="_blank" rel="noreferrer" className="text-[14px] font-semibold text-primary hover:underline flex items-center gap-1 mt-1">
                      <Feather.FileText className="w-4 h-4" /> View PDF
                    </a>
                  ) : (
                    <span className="text-[14px] font-semibold text-slate-800 dark:text-slate-200">-</span>
                  )}
