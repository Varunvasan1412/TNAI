...
const SnaOfficeBearers = () => {
  const navigate = useNavigate();
  const { data, fetchAll, loading, deleteItem } = useSnaOfficeBearerStore();
  ...
  return (
    <div className="relative space-y-7 min-h-screen">
      ...
      <div className="relative z-10">
        <PageTitle title="SnaOfficeBearers" breadcrumbs={[{ label: 'CRM', path: '/admin/dashboard' }, { label: 'SnaOfficeBearers', active: true }]} />
      </div>
        ...
        <Card className="overflow-hidden bg-white/75 dark:bg-slate-800/65 backdrop-blur-2xl">
          <CardHeader>
            <div className="flex justify-between items-center">
              <CardTitle>Manage SnaOfficeBearers</CardTitle>
              <button onClick={() => navigate('/admin/sna-office-bearers/add')} className="flex items-center gap-1.5 h-9 px-4 text-white text-[13px] font-bold rounded-xl shadow-lg" style={{ background: 'linear-gradient(135deg, var(--color-primary), var(--color-secondary))' }}>
            ...
            <TableContainer>
              ...
              <TBody>
                {loading ? <TR><TD colSpan={6} className="text-center py-10">Loading...</TD></TR> : data.map((item, idx) => (
                  <TR key={item.id}>
                    <TD>{idx + 1}</TD>
                    <TD className="font-bold text-primary">{Array.isArray(item.bearer_names) ? item.bearer_names.join(', ') : (item.bearer_names || '-')}</TD>
                    <TD>{item.designation || '-'}</TD>
                    <TD>{item.academic_year || '-'}</TD>
                    <TD><Badge variant={item.status === 'active' ? 'soft-success' : 'soft-danger'}>{item.status || 'Active'}</Badge></TD>
                    <TD className="text-right">
                      <div className="flex justify-end gap-1">
                        <button onClick={() => navigate(`/admin/sna-office-bearers/view/${item.id}`)} className="w-8 h-8 flex justify-center items-center hover:bg-info/10 text-gray-400 hover:text-info rounded-lg"><Feather.Eye className="w-4 h-4" /></button>
                        <button onClick={() => navigate(`/admin/sna-office-bearers/edit/${item.id}`)} className="w-8 h-8 flex justify-center items-center hover:bg-primary/10 text-gray-400 hover:text-primary rounded-lg"><Feather.Edit2 className="w-4 h-4" /></button>
...
export default SnaOfficeBearers;
