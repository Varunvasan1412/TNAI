...
const ImpactView = () => {
  ...
  const { single, fetchSingle, loading } = useImpactStore();
  ...
  return (
    <div className="relative space-y-7 min-h-screen">
      ...
      <div className="relative z-10 flex items-center justify-between">
        <PageTitle title="View Record" breadcrumbs={[{ label: 'CRM', path: '/admin/dashboard' }, { label: 'Impacts', path: '/admin/impacts' }, { label: 'Details', active: true }]} />
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={() => navigate('/admin/impacts')}><Feather.ArrowLeft className="w-4 h-4 mr-2" /> Back</Button>
          <Button size="sm" onClick={() => navigate(`/admin/impacts/edit/${id}`)} style={{ background: 'linear-gradient(135deg, var(--color-primary), var(--color-secondary))', color: 'white', border: 'none' }}>
            ...
            <CardBody className="p-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                ...
                <div className="flex flex-col p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                  <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">Impact Title</span>
                ...
                <div className="md:col-span-2 flex flex-col p-4 rounded-xl bg-slate-50 dark:bg-slate-700/30 border border-slate-100 dark:border-slate-700/50">
                  <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2">Short Summary</span>
                  <p className="text-[14px] font-medium text-slate-700 dark:text-slate-300 whitespace-pre-wrap leading-relaxed">{single.short_summary || '-'}</p>
                </div>
                <div className="md:col-span-2 flex flex-col p-4 rounded-xl bg-slate-50 dark:bg-slate-700/30 border border-slate-100 dark:border-slate-700/50">
                  <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2">Detailed Description</span>
                  <p className="text-[14px] font-medium text-slate-700 dark:text-slate-300 whitespace-pre-wrap leading-relaxed" dangerouslySetInnerHTML={{__html: single.detailed_description || '-'}}></p>
                </div>
                <div className="flex flex-col p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                  <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">Impact Category</span>
                  <span className="text-[14px] font-semibold text-slate-800 dark:text-slate-200">{single.category || '-'}</span>
                </div>
                <div className="flex flex-col p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                  <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">Impact Date</span>
                  <span className="text-[14px] font-semibold text-slate-800 dark:text-slate-200">{single.impact_date || '-'}</span>
                </div>
                <div className="flex flex-col p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                  <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">Location</span>
                  <span className="text-[14px] font-semibold text-slate-800 dark:text-slate-200">{single.location || '-'}</span>
                </div>
                <div className="flex flex-col p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                  <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">Beneficiaries/People Impacted</span>
                  <span className="text-[14px] font-semibold text-slate-800 dark:text-slate-200">{single.beneficiaries_count ? single.beneficiaries_count.toLocaleString() : '-'}</span>
                </div>
                <div className="flex flex-col p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                  <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">Featured Image</span>
                  {single.featured_image ? (
                    <img src={single.featured_image} alt={single.title} className="mt-1 h-16 w-auto rounded border" />
                  ) : (
                    <span className="text-[14px] font-semibold text-slate-800 dark:text-slate-200">-</span>
                  )}
                </div>
                <div className="md:col-span-2 flex flex-col p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                  <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">Supporting Images</span>
                  {single.supporting_images && single.supporting_images.length > 0 ? (
                    <div className="flex flex-wrap gap-2 mt-1">
                       {single.supporting_images.map((img, i) => (
                         <img key={i} src={img} alt={`Supporting ${i+1}`} className="h-16 w-auto rounded border" />
                       ))}
                    </div>
                  ) : (
                    <span className="text-[14px] font-semibold text-slate-800 dark:text-slate-200">-</span>
                ...
                <div className="flex flex-col p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                  <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">Supporting Document</span>
                  {single.supporting_document ? (
                    <a href={single.supporting_document} target="_blank" rel="noreferrer" className="text-[14px] font-semibold text-primary hover:underline flex items-center gap-1 mt-1">
                      <Feather.FileText className="w-4 h-4" /> View Document
                    </a>
                  ) : (
                    <span className="text-[14px] font-semibold text-slate-800 dark:text-slate-200">-</span>
                  )}
                ...
                <div className="flex flex-col p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                  <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">External URL</span>
                  {single.external_url ? (
                    <a href={single.external_url} target="_blank" rel="noreferrer" className="text-[14px] font-semibold text-primary hover:underline truncate w-full block mt-1">
                      {single.external_url}
                    </a>
                  ) : (
                    <span className="text-[14px] font-semibold text-slate-800 dark:text-slate-200">-</span>
