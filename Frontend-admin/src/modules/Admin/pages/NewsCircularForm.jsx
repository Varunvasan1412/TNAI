...
const NewsCircularForm = () => {
  const { id } = useParams();
  const navigate = useNavigate();
  const isEdit = Boolean(id);
  const { single, fetchSingle, createItem, updateItem, loading } = useNewsCircularStore();
  ...
  const handleSubmit = async (e) => {
    ...
    // Convert string to boolean if needed, though checkbox handles it.
    // Convert display_order to integer.
    const payload = {
      ...formData,
      display_order: formData.display_order ? parseInt(formData.display_order, 10) : 0,
      display_on_homepage: formData.display_on_homepage === 'true' || formData.display_on_homepage === true
    };
    
    if (isEdit) {
      success = await updateItem(id, payload);
    } else {
      success = await createItem(payload);
    }
    
    if (success) {
      navigate('/admin/news-circulars');
    }
  };
  ...
  return (
    <div className="relative space-y-7 min-h-screen">
      <div className="relative z-10 flex items-center justify-between">
        <PageTitle title={isEdit ? "Edit Record" : "Add New Record"} breadcrumbs={[{ label: 'CRM', path: '/admin/dashboard' }, { label: 'News & Circulars', path: '/admin/news-circulars' }, { label: isEdit ? 'Edit' : 'Add', active: true }]} />
          ...
          <CardBody className="p-8">
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div><label className="block text-[13px] font-bold mb-1.5">Content Type</label><select name="content_type" value={formData.content_type || ''} onChange={handleChange} className={inputClass}><option value="">Select...</option><option value="news">News</option><option value="circular">Circular</option></select></div>
                <div><label className="block text-[13px] font-bold mb-1.5">Title</label><input type="text" name="title" value={formData.title || ''} onChange={handleChange} className={inputClass} /></div>
                ...
                <div className="md:col-span-2 lg:col-span-3"><label className="block text-[13px] font-bold mb-1.5">Full Content</label><textarea name="full_content" value={formData.full_content || ''} onChange={handleChange} className={textareaClass} /></div>
                ...
                <div><label className="block text-[13px] font-bold mb-1.5">Display on Homepage</label><select name="display_on_homepage" value={formData.display_on_homepage} onChange={handleChange} className={inputClass}><option value={true}>Yes</option><option value={false}>No</option></select></div>
              ...
              <div className="flex justify-end gap-3 pt-6 border-t border-gray-100">
                <Button type="button" variant="outline" onClick={() => navigate('/admin/news-circulars')}>Cancel</Button>
...
export default NewsCircularForm;
