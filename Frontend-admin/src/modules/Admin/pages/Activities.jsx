...
const Activities = () => {
  ...
  const { data, fetchAll, loading, deleteItem } = useActivityStore();
  ...
  return (
    <div className="relative space-y-7 min-h-screen">
      ...
      <div className="relative z-10">
        <PageTitle title="Activities" breadcrumbs={[{ label: 'CRM', path: '/admin/dashboard' }, { label: 'Activities', active: true }]} />
      </div>
        ...
        <Card className="overflow-hidden bg-white/75 dark:bg-slate-800/65 backdrop-blur-2xl">
          <CardHeader>
            <div className="flex justify-between items-center">
              ...
              <button onClick={() => navigate('/admin/activities/add')} className="flex items-center gap-1.5 h-9 px-4 text-white text-[13px] font-bold rounded-xl shadow-lg" style={{ background: 'linear-gradient(135deg, var(--color-primary), var(--color-secondary))' }}>
                  ...
                  <TR key={item.id}>
                    ...
                    <TD className="text-right">
                      <div className="flex justify-end gap-1">
                        <button onClick={() => navigate(`/admin/activities/view/${item.id}`)} className="w-8 h-8 flex justify-center items-center hover:bg-info/10 text-gray-400 hover:text-info rounded-lg"><Feather.Eye className="w-4 h-4" /></button>
                        <button onClick={() => navigate(`/admin/activities/edit/${item.id}`)} className="w-8 h-8 flex justify-center items-center hover:bg-primary/10 text-gray-400 hover:text-primary rounded-lg"><Feather.Edit2 className="w-4 h-4" /></button>
