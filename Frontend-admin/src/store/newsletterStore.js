...
export const useNewsletterStore = create((set, get) => ({
  ...
  fetchAll: async () => {
    set({ loading: true });
    try {
      const response = await axiosInstance.get('/api/tnai/admin/newsletters');
  ...
  fetchSingle: async (id) => {
    ...
    try {
      const response = await axiosInstance.get(`/api/tnai/admin/newsletters/${id}`);
  ...
  createItem: async (data) => {
    set({ loading: true });
    try {
      const response = await axiosInstance.post('/api/tnai/admin/newsletters', data);
      set((state) => ({ 
  ...
  updateItem: async (id, data) => {
    set({ loading: true });
    try {
      const response = await axiosInstance.put(`/api/tnai/admin/newsletters/${id}`, data);
  ...
  deleteItem: async (id) => {
    set({ loading: true });
    try {
      await axiosInstance.delete(`/api/tnai/admin/newsletters/${id}`);
      set((state) => ({ 
